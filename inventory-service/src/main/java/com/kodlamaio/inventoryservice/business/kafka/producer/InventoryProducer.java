...
public class InventoryProducer {
    ...
    log.info(String.format("car_created event => %s",event.toString()));
        Message<CarCreatedEvent> message= MessageBuilder
                .withPayload(event)
                .setHeader(KafkaHeaders.TOPIC, "car_created")
                .build();
    ...
    public void sendMessage(CarDeletedEvent event){
        log.info(String.format("car_delete event => %s",event.toString()));
        Message<CarDeletedEvent> message= MessageBuilder
                .withPayload(event)
                .setHeader(KafkaHeaders.TOPIC, "car_deleted")
                .build();
        kafkaTemplate.send(message);
    }
    public void sendMessage(BrandDeletedEvent event){
        log.info(String.format("brand_delete event => %s",event.toString()));
        Message<BrandDeletedEvent> message= MessageBuilder
                .withPayload(event)
                .setHeader(KafkaHeaders.TOPIC, "brand_deleted")
                .build();
        kafkaTemplate.send(message);
    }
}
